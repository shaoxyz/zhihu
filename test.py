...
"""
#by egrcc
url https://github.com/egrcc/zhihu-python
"""

from zhihuSpider import Question
from zhihuSpider import Answer
from zhihuSpider import User
from zhihuSpider import Collection_zh
from zhihuSpider import Post
from zhihuSpider import Column
...
def question_test(url):
    ...
    # 题和彼题共同的“心愿“吧。
    print "回答数:%s" % answers_num  # 输出：9092
    print "关注数:%s" % followers_num  # 输出：127049
    print topics  # 输出：生活 现实 社会 经历
    print "浏览数:%s" % visit_times  # 输出: 6149968
...
def answer_test(answer_url):
    ...
    ## 获取改该答案所属问题被浏览次数（2017/7/4接口失效）
    #visit_times = answer.get_visit_times()
    ## 获取所有给该答案点赞的用户信息（2017/7/4接口失效）
...
def collection_test(collection_url):
    collection = Collection_zh(collection_url)
...
def main():
    #url = "http://www.zhihu.com/question/24269892"
    #question_test(url)
    #answer_url = "https://www.zhihu.com/question/55166678/answer/143332790"
    #answer_test(answer_url)
    user_url = "http://www.zhihu.com/people/shaowanbo"
    user_test(user_url)
